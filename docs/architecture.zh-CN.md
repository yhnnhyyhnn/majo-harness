...
majo-web/         WebMain（JDK HttpServer 承载已启动树 + 静态聊天 UI）
                  / web.yml profile
docs/             本文档（中英双语）
```
...
| `sandbox/pre-confine` | waterfall | `(List<String> argv)` | 包裹前的策略；抛 `SandboxException` 即拒绝 |
| `interaction/approval-request` / `interaction/approval` | emit | `(ApprovalRequest)` / `(ApprovalRequest, ApprovalDecision)` | 审批进入与决议，供实时观察者 |
| `interaction/question` / `interaction/answer` | emit | `(Question)` / `(Question, String)` | ask-user 进入与答案 |
...
- **M4（进行中）** — 分发：`majo-cli` shaded 启动器（`majo "task"`、`--profile`、退出码）、外部插件 jar（jcordis SPI + 隔离类加载器，热 `replaceJar`/`unload`，`--plugin`）、`majo-web` profile 应用（dsh 风格聊天 UI + JSON turn API，由 JDK HttpServer 承载；以外部客户端（如浏览器）方式体验）。剩余：在 `HarnessBoot` 之上做 profile/bundle 分层与 patch、SDK 面、system-prompt 分段组装、并发 subagent 的 per-agent ctx。
